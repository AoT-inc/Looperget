#!/bin/bash
#
#  setup_unattended.sh - Looperget install script (unattended version)
#
#  Usage: sudo /bin/bash /opt/Looperget/install/setup_unattended.sh [influx-option]
...
case "${1:-''}" in
    '1')
        printf "#### Installing Looperget with Influxdb 1.x\n"
    ;;
    '2')
        printf "#### Installing Looperget with Influxdb 2.x\n"
    ;;
    '0')
        printf "#### Installing Looperget without installing Influxdb\n"
    ;;
    *)
...
if hash python3 2>/dev/null; then
  if ! python3 "${INSTALL_DIRECTORY}"/looperget/scripts/upgrade_check.py --min_python_version "3.8"; then
...
NOW=$(date)
printf "### Looperget installation initiated %s\n\n" "${NOW}" 2>&1 | tee -a "${LOG_LOCATION}"

abort()
{
    printf "
**********************************
** ERROR During Looperget Install! **
**********************************

An error occurred that may have prevented Looperget from
...
SECONDS=0
NOW=$(date)
printf "#### Looperget installation began %s\n" "${NOW}" 2>&1 | tee -a "${LOG_LOCATION}"
...
CURRENT_DATE=$(date)
printf "#### Looperget Installer finished %s\n" "${CURRENT_DATE}" 2>&1 | tee -a "${LOG_LOCATION}"
