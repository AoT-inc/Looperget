...
case "${1:-''}" in
    ...
    ;;
    "test")
        docker exec -ti looperget_flask "${INSTALL_PATH}"/env/bin/python -m pip install --upgrade -r /home/looperget/looperget/install/requirements-testing.txt
        docker exec -ti looperget_flask "${INSTALL_PATH}"/env/bin/python -m pytest /home/looperget/looperget/tests/software_tests
    ;;
    "clean-all")
