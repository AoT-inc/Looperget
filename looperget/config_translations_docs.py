# -*- coding: utf-8 -*-
#
#  config_translations_docs.py - Looperget Docs phrases for translation
#
...
TRANSLATIONS = {
    "index.md": {
        'Looperget Environmental Monitoring and Regulation System': lazy_gettext('Looperget Environmental Monitoring and Regulation System'),
        'text_1_1': lazy_gettext('Looperget is open source software designed to run on the [Raspberry Pi](https://en.wikipedia.org/wiki/Raspberry_Pi) and other single-board computers (SBCs). It couples inputs and outputs in interesting ways to sense and manipulate the environment.'),

        'Information': lazy_gettext('Information'),
        'text_2_1': lazy_gettext('See the [README](https://github.com/aot-inc/Looperget#uses) for features, projects using Looperget, screenshots, and other information.'),
        ...
        'text_4_1': lazy_gettext('Once booted and logged in, run the following command to initiate the Looperget install:'),
    ...
    "About.md": {
        "text_1_1": lazy_gettext("Looperget is an open-source environmental monitoring and regulation system that was built to run on single-board computers, specifically the [Raspberry Pi](https://en.wikipedia.org/wiki/Raspberry_Pi)."),
        ...
        "text_1_3": lazy_gettext("There are a number of different uses for Looperget. Some users simply store sensor measurements to monitor conditions remotely, others regulate the environmental conditions of a physical space, while others capture motion-activated or time-lapse photography, among other uses."),
        ...
        "text_1_8": lazy_gettext("Looperget has been translated to several languages. By default, the language of the browser will determine which language is used, but may be overridden in the General Settings, on the `[Gear Icon] -> Configure -> General` page. If you find an issue and would like to correct a translation or would like to add another language, this can be done at [https://translate.aot-inc.com](https://translate.aot-inc.com/engage/looperget/).")
    },

    "Data-Viewing.md": {
        "Live Measurements": lazy_gettext("Live Measurements"),
        "text_1_1": lazy_gettext("The `Live Measurements` page is the first page a user sees after logging in to Looperget. It will display the current measurements being acquired from Input and Function controllers. If there is nothing displayed on the `Live` page, ensure an Input or Function controller is both configured correctly and activated. Data will be automatically updated on the page from the measurement database."),
        ...
        "text_5_1": lazy_gettext("There is a Custom Widget import system in Looperget that allows user-created Widgets to be used in the Looperget system. Custom Widgets can be uploaded on the `[Gear Icon] -> Configure -> Custom Widgets` page. After import, they will be available to use on the `Setup -> Widget` page."),
        "text_5_2": lazy_gettext("If you develop a working module, please consider [creating a new GitHub issue](https://github.com/aot-inc/Looperget/issues/new?assignees=&labels=&template=feature-request.md&title=New%20Module) or pull request, and it may be included in the built-in set."),
        "text_5_3": lazy_gettext("Open any of the built-in Widget modules located in the directory [Looperget/looperget/widgets](https://github.com/aot-inc/Looperget/tree/master/looperget/widgets/) for examples of the proper formatting. There are also example Custom Widgets in the directory [Looperget/looperget/widgets/examples](https://github.com/aot-inc/Looperget/tree/master/looperget/widgets/examples)."),
