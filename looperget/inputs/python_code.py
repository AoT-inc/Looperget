...
from looperget.utils.actions import run_input_actions
from looperget.utils.database import db_retrieve_table_daemon
...
def execute_at_modification(
    ...
    try:
        ...
        cmd_test = 'mkdir -p /opt/Looperget/.pylint.d && ' \
                   'export PYTHONPATH=$PYTHONPATH:/opt/Looperget && ' \
                   'export PYLINTHOME=/opt/Looperget/.pylint.d && ' \
...
INPUT_INFORMATION = {
    ...
    'message': 'All channels require a Measurement Unit to be selected and saved in order to store values to the '
               'database. Your code is executed from the same Python virtual environment that Looperget runs from. '
               'Therefore, you must install Python libraries to this environment if you want them to be available to '
               'your code. This virtualenv is located at /opt/Looperget/env and if you wanted to install a library, for '
               'example "my_library" using pip, you would execute "sudo /opt/Looperget/env/bin/pip install my_library".',
...
class InputModule(AbstractInput):
    ...
    def get_measurement(self):
        ...
        if not self.python_code:
            self.logger.error("Error 101: Device not set up. See https://aot-inc.github.io/Looperget/Error-Codes#error-101 for more info.")
            return
        ...
        module_name = "looperget.input.python_code_exec_{}".format(os.path.basename(file_run).split('.')[0])
