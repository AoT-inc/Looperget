...
from looperget.databases import CRUDMixin, set_uuid
from looperget.looperget_flask.extensions import db

logger = logging.getLogger("looperget.misc")
...
class Misc(CRUDMixin, db.Model):
    ...
    enable_upgrade_check = db.Column(db.Boolean, default=True)  # Periodically check for a Looperget upgrade
    looperget_upgrade_available = db.Column(db.Boolean, default=False)  # Stores if an upgrade is available
