# -*- coding: utf-8 -*-
#
# config.py - Global Looperget settings
#
import binascii
import sys
...
# Set True to enable upgrading to the master branch of the Looperget repository.
...
# Accessed at https://127.0.0.1/looperget-flask-profiler
...
try:
    ...
    LOOPERGET_DB_PATH = config_override.LOOPERGET_DB_PATH
except:
    LOOPERGET_DB_PATH = f'sqlite:///{SQL_DATABASE_LOOPERGET}'

# Misc paths
PATH_1WIRE = '/sys/bus/w1/devices/'
PATH_CONTROLLERS = os.path.join(INSTALL_DIRECTORY, 'looperget/controllers')
PATH_FUNCTIONS = os.path.join(INSTALL_DIRECTORY, 'looperget/functions')
PATH_ACTIONS = os.path.join(INSTALL_DIRECTORY, 'looperget/actions')
PATH_INPUTS = os.path.join(INSTALL_DIRECTORY, 'looperget/inputs')
PATH_OUTPUTS = os.path.join(INSTALL_DIRECTORY, 'looperget/outputs')
PATH_WIDGETS = os.path.join(INSTALL_DIRECTORY, 'looperget/widgets')
...
PATH_TEMPLATE = os.path.join(INSTALL_DIRECTORY, 'looperget/looperget_flask/templates')
...
PATH_USER_SCRIPTS = os.path.join(INSTALL_DIRECTORY, 'looperget/user_scripts')
PATH_PYTHON_CODE_USER = os.path.join(INSTALL_DIRECTORY, 'looperget/user_python_code')
PATH_MEASUREMENTS_BACKUP = os.path.join(INSTALL_DIRECTORY, 'looperget/backup_measurements')
PATH_SETTINGS_BACKUP = os.path.join(INSTALL_DIRECTORY, 'looperget/backup_settings')
...
BACKUP_PATH = '/var/Looperget-backups'  # Where Looperget backups are stored

# Log files
LOG_PATH = '/var/log/looperget'  # Where generated logs are stored
LOGIN_LOG_FILE = os.path.join(LOG_PATH, 'login.log')
DAEMON_LOG_FILE = os.path.join(LOG_PATH, 'looperget.log')
KEEPUP_LOG_FILE = os.path.join(LOG_PATH, 'loopergetkeepup.log')
BACKUP_LOG_FILE = os.path.join(LOG_PATH, 'loopergetbackup.log')
DEPENDENCY_LOG_FILE = os.path.join(LOG_PATH, 'loopergetdependency.log')
IMPORT_LOG_FILE = os.path.join(LOG_PATH, 'loopergetimport.log')
UPGRADE_LOG_FILE = os.path.join(LOG_PATH, 'loopergetupgrade.log')
UPGRADE_TMP_LOG_FILE = '/tmp/loopergetupgrade.log'
RESTORE_LOG_FILE = os.path.join(LOG_PATH, 'loopergetrestore.log')
HTTP_ACCESS_LOG_FILE = '/var/log/nginx/access.log'
HTTP_ERROR_LOG_FILE = '/var/log/nginx/error.log'

# Lock files
LOCK_PATH = '/var/lock'
LOCK_FILE_STREAM = os.path.join(LOCK_PATH, 'looperget-camera-stream.pid')

# Run files
RUN_PATH = '/var/run'
FRONTEND_PID_FILE = os.path.join(RUN_PATH, 'loopergetflask.pid')
DAEMON_PID_FILE = os.path.join(RUN_PATH, 'looperget.pid')

# Remote admin
STORED_SSL_CERTIFICATE_PATH = os.path.join(
    INSTALL_DIRECTORY, 'looperget/looperget_flask/ssl_certs/remote_admin')
...
# Pyro5 URI/host, used by looperget_client.py
if DOCKER_CONTAINER:
    PYRO_URI = 'PYRO:looperget.pyro_server@looperget_daemon:9080'
else:
    PYRO_URI = 'PYRO:looperget.pyro_server@127.0.0.1:9080'

# Anonymous statistics
STATS_INTERVAL = 86400
STATS_HOST = 'fungi.aot-inc.com'
STATS_PORT = 8086
STATS_USER = 'looperget_stats'
STATS_PASSWORD = 'Io8Nasr5JJDdhPOj32222'
STATS_DATABASE = 'looperget_stats'
...
TAGS_URL = 'https://api.github.com/repos/aot-inc/Looperget/git/refs/tags'
...
METHOD_DEP_BASE = [
    ('bash-commands',
     ['/opt/Looperget/looperget/looperget_flask/static/js/user_js/highcharts-9.1.2.js'],
     [
        'wget --no-clobber https://code.highcharts.com/zips/Highcharts-9.1.2.zip',
        'unzip Highcharts-9.1.2.zip -d Highcharts-9.1.2',
        'cp -rf Highcharts-9.1.2/code/highcharts.js /opt/Looperget/looperget/looperget_flask/static/js/user_js/highcharts-9.1.2.js',
        'cp -rf Highcharts-9.1.2/code/highcharts.js.map /opt/Looperget/looperget/looperget_flask/static/js/user_js/highcharts.js.map',
...
class ProdConfig(object):
    ...
    SQL_DATABASE_LOOPERGET = os.path.join(DATABASE_PATH, DATABASE_NAME)
    LOOPERGET_DB_PATH = f'sqlite:///{SQL_DATABASE_LOOPERGET}'
    SQLALCHEMY_DATABASE_URI = f'sqlite:///{SQL_DATABASE_LOOPERGET}'
    ...
    FLASK_PROFILER = {
        ...
        "endpointRoot": "looperget-flask-profiler"
    }
