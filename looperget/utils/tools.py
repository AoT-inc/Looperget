...
from looperget.config import (INSTALL_DIRECTORY, PATH_ACTIONS_CUSTOM,
                           PATH_FUNCTIONS_CUSTOM, PATH_TEMPLATE_USER,
                           PATH_INPUTS_CUSTOM, PATH_OUTPUTS_CUSTOM,
                           PATH_PYTHON_CODE_USER, PATH_USER_SCRIPTS,
                           PATH_WIDGETS_CUSTOM, SQL_DATABASE_LOOPERGET,
                           USAGE_REPORTS_PATH)
from looperget.databases.models import (Conversion, DeviceMeasurements,
                                     EnergyUsage, Misc, Output, OutputChannel)
from looperget.utils.database import db_retrieve_table_daemon
from looperget.utils.influx import (average_past_seconds,
...
logger = logging.getLogger("looperget.tools")
...
def create_measurements_export(influxdb_version, save_path=None):
    try:
        ...
        if influxdb_version.startswith("1."):
            ...
            cmd = f"/usr/bin/influx backup --org looperget {influx_backup_dir}"
            out, err, status = cmd_output(cmd, user='root')
        else:
        ...
        if not status:
            ...
            if save_path:
                ...
                set_user_grp(save_path, 'looperget', 'looperget')
                return 0, save_path
            else:
                return 0, data
...
def create_settings_export(save_path=None):
    try:
        ...
        with zipfile.ZipFile(data, mode='w') as z:
            try:
                z.write(SQL_DATABASE_LOOPERGET,
                        os.path.basename(SQL_DATABASE_LOOPERGET))
            except:
                logger.error(f"Could not find database file {SQL_DATABASE_LOOPERGET}")
...
def generate_output_usage_report():
    ...
    try:
        ...
        set_user_grp(report_path_file, 'looperget', 'looperget')
    except Exception:
