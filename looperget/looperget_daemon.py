#!/opt/Looperget/env/bin/python
# -*- coding: utf-8 -*-
#
#  looperget_daemon.py - Daemon for managing Looperget controllers, such as sensors,
...
from looperget.config import (DAEMON_LOG_FILE, DOCKER_CONTAINER, LOOPERGET_DB_PATH,
                           LOOPERGET_VERSION, STATS_CSV, STATS_INTERVAL,
                           UPGRADE_CHECK_INTERVAL)
from looperget.controllers.controller_conditional import ConditionalController
from looperget.controllers.controller_function import FunctionController
from looperget.controllers.controller_input import InputController
from looperget.controllers.controller_output import OutputController
from looperget.controllers.controller_pid import PIDController
from looperget.controllers.controller_trigger import TriggerController
from looperget.controllers.controller_widget import WidgetController
from looperget.databases.models import (PID, Camera, Conditional,
...
from looperget.utils.actions import (get_condition_value,
...
from looperget.utils.github_release_info import LoopergetRelease
from looperget.utils.stats import (add_update_csv, recreate_stat_file,
                                return_stat_file_dict, send_anonymous_stats)
from looperget.utils.tools import generate_output_usage_report, next_schedule
...
logger = logging.getLogger('looperget')
...
class DaemonController:
    ...
    def __init__(self):
        ...
        self.logger.info(f"Looperget daemon v{LOOPERGET_VERSION} starting")
    ...
    def run(self):
        ...
        self.logger.info(
            f"Looperget daemon started in {self.startup_time:.3f} seconds")
        ...
        while self.daemon_run:
            ...
            try:
                ...
                if self.enable_upgrade_check and now > self.timer_upgrade:
                    ...
                    self.check_looperget_upgrade_exists(now)
        ...
        self.logger.info(f"Looperget daemon terminated in {timer:.3f} seconds\n\n")
    ...
    def check_looperget_upgrade_exists(self, now):
        """Check for any new Looperget releases on github."""
        try:
            looperget_releases = LoopergetRelease()
            (upgrade_exists, _, _, _, errors) = looperget_releases.github_upgrade_exists()
            ...
            if upgrade_exists:
                ...
                if now > self.timer_upgrade_message:
                    ...
                    self.logger.info(
                        "A new version of Looperget is available. Upgrade "
                        "through the web interface under Config -> Upgrade. "
                        "This message will repeat every 10 days unless "
                        "Looperget is upgraded or upgrade checks are disabled.")
            else:
                upgrade_available = False

            with session_scope(LOOPERGET_DB_PATH) as new_session:
                mod_misc = new_session.query(Misc).first()
                if mod_misc.looperget_upgrade_available != upgrade_available:
                    mod_misc.looperget_upgrade_available = upgrade_available
                    new_session.commit()
        except Exception:
            self.logger.exception("Looperget Upgrade Check ERROR")
...
class PyroServer(object):
    ...
    """
    def __init__(self, looperget):
        self.looperget = looperget

    def lcd_reset(self, lcd_id):
        """Resets an LCD."""
        return self.looperget.lcd_reset(lcd_id)

    def lcd_backlight(self, lcd_id, state):
        """Turns an LCD backlight on or off."""
        return self.looperget.lcd_backlight(lcd_id, state)

    def display_backlight_color(self, lcd_id, color):
        """Set the LCD backlight color."""
        return self.looperget.display_backlight_color(lcd_id, color)

    def lcd_flash(self, lcd_id, state):
        """Starts or stops an LCD from flashing (alarm)"""
        return self.looperget.lcd_flash(lcd_id, state)

    def get_condition_measurement(self, condition_id):
        return self.looperget.get_condition_measurement(condition_id)

    def get_condition_measurement_dict(self, condition_id):
        return self.looperget.get_condition_measurement_dict(condition_id)

    def module_function(self, controller_type, unique_id, button_id, args_dict, thread=True, return_from_function=False):
        """execute custom button function."""
        return self.looperget.module_function(
            controller_type, unique_id, button_id, args_dict, thread=thread, return_from_function=return_from_function)

    def controller_activate(self, cont_id):
        """Activates a controller."""
        try:
            return self.looperget.controller_activate(cont_id)
        except Exception as except_msg:
    ...
    def controller_deactivate(self, cont_id):
        ...
        try:
            return self.looperget.controller_deactivate(cont_id)
        except Exception as except_msg:
    ...
    def controller_restart(self, cont_id):
        ...
        try:
            return self.looperget.controller_restart(cont_id)
        except Exception as except_msg:
    ...
    def controller_is_active(self, cont_id):
        """Checks if a controller is active."""
        return self.looperget.controller_is_active(cont_id)

    def check_daemon(self):
        """Check if all active controllers respond."""
        return self.looperget.check_daemon()

    def function_status(self, function_id):
        """Get status of Function."""
        return self.looperget.function_status(function_id)

    def input_force_measurements(self, input_id):
        """Updates all input information."""
        return self.looperget.input_force_measurements(input_id)

    def pid_hold(self, pid_id):
        """Hold PID Controller operation."""
        return self.looperget.pid_hold(pid_id)

    def pid_mod(self, pid_id):
        """Set new PID Controller settings."""
        return self.looperget.pid_mod(pid_id)

    def pid_pause(self, pid_id):
        """Pause PID Controller operation."""
        return self.looperget.pid_pause(pid_id)

    def pid_resume(self, pid_id):
        """Resume PID controller operation."""
        return self.looperget.pid_resume(pid_id)

    def pid_get(self, pid_id, setting):
        """Get PID setting."""
        return self.looperget.pid_get(pid_id, setting)

    def pid_set(self, pid_id, setting, value):
        """Set PID setting."""
        return self.looperget.pid_set(pid_id, setting, value)

    def refresh_daemon_conditional_settings(self, unique_id):
        """Instruct the daemon to refresh a conditional's settings."""
        return self.looperget.refresh_daemon_conditional_settings(unique_id)

    def refresh_daemon_misc_settings(self):
        """Instruct the daemon to refresh the misc settings."""
        return self.looperget.refresh_daemon_misc_settings()

    def refresh_daemon_trigger_settings(self, unique_id):
        """Instruct the daemon to refresh a conditional's settings."""
        return self.looperget.refresh_daemon_trigger_settings(unique_id)

    def output_state(self, output_id, output_channel):
        """Return the output state (on or off)"""
        return self.looperget.output_state(output_id, output_channel)

    def output_states_all(self):
        """Return all output states."""
        return self.looperget.output_states_all()
    ...
    def output_on(self,
        ...
        return self.looperget.output_on(
            output_id,
            output_channel=output_channel,
            output_type=output_type,
            amount=amount,
    ...
    def output_off(self, output_id, output_channel=None, trigger_conditionals=True):
        ...
        return self.looperget.output_off(
    ...
    def output_sec_currently_on(self, output_id, output_channel=None):
        ...
        return self.looperget.controller['Output'].output_sec_currently_on(
            output_id, output_channel=output_channel)

    def output_setup(self, action, output_id):
        """Add, delete, or modify a output in the running output controller."""
        return self.looperget.output_setup(action, output_id)

    def trigger_action(self, action_id, value={}, debug=False):
        """Trigger action."""
        return self.looperget.trigger_action(
            action_id,
            value=value,
            debug=debug)
    ...
    def trigger_all_actions(self, function_id, message='', debug=False):
        """Trigger all actions."""
        return self.looperget.trigger_all_actions(function_id, message=message, debug=debug)

    def terminate_daemon(self):
        """Instruct the daemon to shut down."""
        return self.looperget.terminate_daemon()

    def widget_add_refresh(self, unique_id):
        """Add or refresh widget object."""
        return self.looperget.controller['Widget'].widget_add_refresh(unique_id)

    def widget_remove(self, unique_id):
        """Remove widget object."""
        return self.looperget.controller['Widget'].widget_remove(unique_id)

    def widget_execute(self, unique_id):
        """Execute widget object."""
        return self.looperget.controller['Widget'].widget_execute(unique_id)
...
class PyroDaemon(threading.Thread):
    ...
    controllers. It allows the client (looperget_client.py to be executed as non-root
    user) to communicate with the daemon (looperget_daemon.py running with root privileges).

    """
    def __init__(self, looperget):
        threading.Thread.__init__(self)

        self.logger = logging.getLogger('looperget.pyro_daemon')
        self.looperget = looperget
    ...
    def run(self):
        try:
            ...
            serve({
                PyroServer(self.looperget): 'looperget.pyro_server',
...
class PyroMonitor(threading.Thread):
    ...
    def __init__(self):
        ...
        self.logger = logging.getLogger('looperget.pyro_monitor')
        self.timer_sec = 1800
        ...
        try:
            ...
            while True:
                ...
                if now > log_timer:
                    ...
                    try:
                        proxy = Proxy("PYRO:looperget.pyro_server@127.0.0.1:9080")
                        proxy.check_daemon()
...
class LoopergetDaemon:
    """
    Handle starting the components of the Looperget Daemon
    """
    def __init__(self, looperget):
        self.logger = logging.getLogger('looperget.daemon')
        self.looperget = looperget

    def start_daemon(self):
        """Start communication and daemon threads."""
        try:
            pd = PyroDaemon(self.looperget)
            pd.daemon = True
            pd.start()
            ...
            self.looperget.run()  # Start daemon thread that manages controllers
        except Exception:
            self.logger.exception("ERROR Starting Looperget Daemon")


def parse_args():
    parser = argparse.ArgumentParser(description='Looperget daemon.')
...
if __name__ == '__main__':
    ...
    looperget_daemon = LoopergetDaemon(daemon_controller)
    looperget_daemon.start_daemon()
