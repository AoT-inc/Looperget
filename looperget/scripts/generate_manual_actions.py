...
if __name__ == "__main__":
    for action_id, action_data in parse_action_information(exclude_custom=True).items():
        ...
        if ('manufacturer' in action_data and
                action_data['manufacturer'] in ['Linux', 'Looperget', 'Raspberry Pi', 'System']):
            ...
            if name_str in looperget_info and 'dependencies_module' in looperget_info[name_str]:
                ...
                looperget_info[name_str]['dependencies_module'].append(action_data['dependencies_module'])
            else:
                # Only one set of dependencies
                looperget_info[name_str] = action_data
                if 'dependencies_module' in action_data:
                    looperget_info[name_str]['dependencies_module'] = [action_data['dependencies_module']]  # turn into list
        else:
    ...
    list_actions = [
        (looperget_info, "Built-In Actions (System)"),
