#!/bin/bash
# Downloads the version of Looperget to upgrade to
...
runDownloadLooperget() {
  INSTALL_DIRECTORY=$( cd -P /var/looperget-root/.. && pwd -P )
  echo '1' > "${INSTALL_DIRECTORY}"/Looperget/.upgrade
  ...
  CURRENT_VERSION=$("${INSTALL_DIRECTORY}"/Looperget/env/bin/python3 "${INSTALL_DIRECTORY}"/Looperget/looperget/utils/github_release_info.py -c 2>&1)
  ...
  if [ "$UPGRADE_TYPE" == "upgrade-release-major" ] || [ "$UPGRADE_TYPE" == "upgrade-release-wipe" ] ; then
    ...
    if [ -n "$UPGRADE_MAJ_VERSION" ]; then
      ...
      UPDATE_VERSION=$("${INSTALL_DIRECTORY}"/Looperget/env/bin/python3 "${INSTALL_DIRECTORY}"/Looperget/looperget/utils/github_release_info.py -m "$UPGRADE_MAJ_VERSION" -v 2>&1)
      UPDATE_URL=$("${INSTALL_DIRECTORY}"/Looperget/env/bin/python3 "${INSTALL_DIRECTORY}"/Looperget/looperget/utils/github_release_info.py -m "$UPGRADE_MAJ_VERSION" 2>&1)
    ...
    UPDATE_URL="https://github.com/aot-inc/Looperget/archive/master.tar.gz"
    TARBALL_FILE="Looperget-master"

  fi

  LOOPERGET_NEW_TMP_DIR="/tmp/Looperget-${UPDATE_VERSION}"
  TARBALL_FILE="looperget-${UPDATE_VERSION}"
  ...
  printf "Downloading latest Looperget version %s to %s/%s.tar.gz..." "${UPDATE_VERSION}" "${INSTALL_DIRECTORY}" "${TARBALL_FILE}"
  ...
  if [ -d "${LOOPERGET_NEW_TMP_DIR}" ] ; then
    printf "The tmp directory %s already exists. Removing..." "${LOOPERGET_NEW_TMP_DIR}"
    if ! rm -rf "${LOOPERGET_NEW_TMP_DIR}" ; then
      printf "Failed: Error while trying to delete tmp directory %s.\n" "${LOOPERGET_NEW_TMP_DIR}"
      error_found
    fi
    printf "Done.\n"
  fi

  printf "Creating %s..." "${LOOPERGET_NEW_TMP_DIR}"
  if ! mkdir "${LOOPERGET_NEW_TMP_DIR}" ; then
    printf "Failed: Error while trying to create %s.\n" "${LOOPERGET_NEW_TMP_DIR}"
    error_found
  fi
  printf "Done.\n"

  printf "Extracting %s/%s.tar.gz to %s..." "${INSTALL_DIRECTORY}" "${TARBALL_FILE}" "${LOOPERGET_NEW_TMP_DIR}"
  if ! tar xzf "${INSTALL_DIRECTORY}"/"${TARBALL_FILE}".tar.gz -C "${LOOPERGET_NEW_TMP_DIR}" --strip-components=1 ; then
    printf "Failed: Error while trying to extract files from %s/%s.tar.gz to %s.\n" "${INSTALL_DIRECTORY}" "${TARBALL_FILE}" "${LOOPERGET_NEW_TMP_DIR}"
    error_found
  else
    if [ "$UPGRADE_TYPE" == "force-upgrade-master" ]; then
      touch "${LOOPERGET_NEW_TMP_DIR}"/.master
    fi
  fi
  printf "Done.\n"
  ...
  exec /bin/bash "${LOOPERGET_NEW_TMP_DIR}"/looperget/scripts/upgrade_install.sh $RELEASE_WIPE
}

runDownloadLooperget "$UPGRADE_TYPE" "$UPGRADE_MAJ_VERSION"
