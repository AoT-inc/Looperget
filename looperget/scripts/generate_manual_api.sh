...
if curl -k -s --head --request GET https://${API_SERV_IP}/api/swagger.json | grep "200 OK" > /dev/null; then
  ...
  then
    ...
    npx redoc-cli bundle -o "${INSTALL_DIRECTORY}"/docs/looperget-api.html /tmp/swagger.json
    rm -rf /tmp/swagger.json

    # Change title
    sed -i 's/<title>ReDoc documentation<\/title>/<title>Looperget API documentation<\/title>/g' "${INSTALL_DIRECTORY}"/docs/looperget-api.html

    cp "${INSTALL_DIRECTORY}"/docs/looperget-api.html "${INSTALL_DIRECTORY}"/looperget/looperget_flask/static/manual/looperget-api.html
  else
