...
from looperget.scripts.generate_doc_output import generate_controller_doc
...
if __name__ == "__main__":
    for output_id, output_data in parse_output_information(exclude_custom=True).items():
        ...
        if ('output_manufacturer' in output_data and
                output_data['output_manufacturer'] in ['Linux', 'Looperget', 'Raspberry Pi', 'System']):

            if name_str in looperget_info and 'dependencies_module' in looperget_info[name_str]:
                # Multiple sets of dependencies, append library
                looperget_info[name_str]['dependencies_module'].append(output_data['dependencies_module'])
            else:
                # Only one set of dependencies
                looperget_info[name_str] = output_data
                if 'dependencies_module' in output_data:
                    looperget_info[name_str]['dependencies_module'] = [output_data['dependencies_module']]  # turn into list
        else:
    ...
    looperget_info = dict(OrderedDict(sorted(looperget_info.items(), key = lambda t: t[0])))
    outputs_info = dict(OrderedDict(sorted(outputs_info.items(), key = lambda t: t[0])))

    list_outputs = [
        (looperget_info, "Built-In Outputs (System)"),
            ...
            for each_id, each_data in each_list[0].items():
                ...
                if 'interfaces' in each_data and each_data['interfaces']:
                    ...
                    for each_type in each_data['interfaces']:
                        ...
                        elif each_type == 'LOOPERGET':
                            list_interfaces.append("Looperget")
