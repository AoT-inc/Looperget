#!/bin/bash
#
#  upgrade_post.sh - Commands to execute after a Looperget upgrade
#
...
# Delete /env if Looperget symlink target isn't /opt/Looperget
if [ "$(readlink /var/looperget-root)" != "/opt/Looperget" ] ; then
...
DB_INFO=$( ${INSTALL_DIRECTORY}/env/bin/python ${INSTALL_DIRECTORY}/looperget/scripts/measurement_db.py -i )
...
TIMER_START_update_looperget_startup_script=$SECONDS
${INSTALL_CMD} update-looperget-startup-script
TIMER_TOTAL_update_looperget_startup_script=$((SECONDS - TIMER_START_update_looperget_startup_script))
...
printf "\nupdate-looperget-startup-script: %s s" "${TIMER_TOTAL_update_looperget_startup_script}"
