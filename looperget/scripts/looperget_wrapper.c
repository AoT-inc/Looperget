...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    if (strcmp(argv[1], "backup-create") == 0) {
      ...
      sprintf(cmd, "rm -rf /var/Looperget-backups/Looperget-backup-%s", argv[2]);
      system(cmd);
      ...
      sprintf(cmd, "service looperget restart");
      system(cmd);
    } else if (strcmp(argv[1], "daemon_start") == 0) {
      sprintf(cmd, "service looperget start");
      system(cmd);
    } else if (strcmp(argv[1], "daemon_stop") == 0) {
      sprintf(cmd, "service looperget stop");
      system(cmd);
    } else if (strcmp(argv[1], "delete_upload_dir") == 0) {
      sprintf(cmd, "rm -rf /opt/Looperget/upload");
      system(cmd);
    } else if (strcmp(argv[1], "frontend_reload") == 0) {
      sprintf(cmd, "service loopergetflask reload");
      system(cmd);
    } else if (strcmp(argv[1], "frontend_restart") == 0) {
      sprintf(cmd, "service loopergetflask restart");
      system(cmd);
      ...
      system(cmd);
    } else if (strcmp(argv[1], "influxdb_restore_looperget_db") == 0 && (argc > 2)) {
      sprintf(cmd, "influxd restore -portable -db looperget_db -newdb looperget_db_bak %s", argv[2]);
      system(cmd);
    ...
    printf("Looperget Wrapper: A wrapper to allow the looperget web interface\n");
    printf("                to perform superuser actions.\n\n");
    printf("Usage: looperget_wrapper [command]\n\n");
    printf("Commands:\n");
    printf("   backup-create:              Create Looperget backup\n");
    printf("   backup-delete [DIRECTORY]:  Delete Looperget backup [DIRECTORY]\n");
    printf("   backup-restore [DIRECTORY]: Restore Looperget from backup [DIRECTORY]\n");
    printf("   daemon_restart:             Restart the looperget daemon in normal mode\n");
    printf("   daemon_restart_debug:       Restart the looperget daemon in debug mode\n");
    printf("   daemon_start:               Start the looperget daemon\n");
    printf("   daemon_stop:                Stop the looperget daemon\n");
    ...
    printf("   frontend_reload:            Reload the looperget frontend\n");
    printf("   frontend_restart:           Restart the looperget frontend\n");
    printf("   influxdb_start:             Start influxdb\n");
    printf("   influxdb_stop:              Stop influxdb\n");
    printf("   influxdb_restart:           Restart influxdb\n");
    printf("   influxdb_restore_looperget_db  Restore influxdb database looperget_db to looperget_db_bak\n");
    printf("   initialize:                 Run the Looperget initialization sequence\n");
    printf("   update_dependencies:        Install unmet dependencies and update installed dependencies\n");
    printf("   update_permissions:         Set the Looperget file/folder permissions\n");
    printf("   restart:                    Restart the computer after a 10 second pause\n");
    printf("   shutdown:                   Shutdown the computer after a 10 second pause\n");
    printf("   upgrade-release-wipe:       Upgrade Looperget to the latest major version on github and wipe database and virtualenv\n");
    printf("   upgrade-release-major:      Upgrade Looperget to the latest major version on github\n");
    printf("   upgrade-master:             Upgrade Looperget to the latest master branch on github\n");
  }
