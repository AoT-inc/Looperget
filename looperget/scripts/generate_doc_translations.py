...
looperget_root = os.path.abspath(os.path.join(__file__, "../../.."))
sys.path.append(looperget_root)

from looperget.config import LANGUAGES
from looperget.config_translations_docs import TRANSLATIONS
...
if __name__ == "__main__":
    for each_doc_file in TRANSLATIONS:
        with open(f'{looperget_root}/docs_templates/{each_doc_file}', 'r') as f:
            contents = f.read()
            contents = replace_contents(contents, _=None, english=True)
            with open(f'{looperget_root}/docs/{each_doc_file}', 'w') as file1:
        ...
        for each_lang in LANGUAGES:
            ...
            translate = gettext.translation(
                'messages',
                localedir=f'{looperget_root}/looperget/looperget_flask/translations',
                languages=[each_lang])
            _ = translate.gettext
            ...
            if contents:
                ...
                with open(f'{looperget_root}/docs/{new_file}', 'w') as file1:
                    file1.write(contents)
