# coding=utf-8
#
# remote_output_pwm.py - Output for controlling a remote Looperget PWM Output
#
import copy
import json
import time
...
from looperget.databases.models import DeviceMeasurements, OutputChannel
from looperget.outputs.base_output import AbstractOutput
from looperget.utils.database import db_retrieve_table_daemon
from looperget.utils.influx import add_measurements_influxdb, read_influxdb_single
...
OUTPUT_INFORMATION = {
    ...
    'output_name': "{} Looperget Output: {}".format(lazy_gettext('Remote'), lazy_gettext('PWM')),
    'measurements_dict': measurements_dict,
    'channels_dict': channels_dict,
    'output_library': 'requests',
    'output_types': ['pwm'],

    'message': 'This Output allows remote control of another Looperget PWM Output over a network using the API.',
    ...
    'custom_options_message': 'Enter the API key and IP/Host address of your remote Looperget and save to populate the Remote Output dropdown selection. You will need to refresh the page after saving for the Remote Looperget Output dropdown to populate. Configure which Remote Looperget Output you would like to control and save again. You must select an On/Off Output Channel for this to work. Selecting a PWM, Volume, or other channel will cause an error.',
    ...
    'custom_options': [
        {
            ...
            'default_value': '',
            'required': True,
            'name': "Remote Looperget Host",
            'phrase': lazy_gettext('The host or IP address of the remote Looperget')
        },
        {
            'id': 'api_key',
            'type': 'text',
            'default_value': '',
            'required': True,
            'name': "Remote Looperget API Key",
            'phrase': lazy_gettext('The API key of the remote Looperget')
        },
        {
    ...
    'custom_channel_options': [
        {
            ...
            'default_value': '',
            'name': 'Remote Looperget Output',
            'phrase': 'The Remote Looperget Output to control'
        },
        {
            'id': 'state_startup',
            'type': 'select',
