...
from looperget.config import FUNCTIONS
from looperget.databases.models import (PID, Actions, Conditional,
                                     CustomController, Dashboard, Function,
                                     Input, Output, Trigger, User, Widget)
from looperget.looperget_flask.utils.utils_general import (choices_custom_functions,
                                                     generate_form_input_list,
                                                     generate_form_output_list,
                                                     generate_form_widget_list)
from looperget.tests.software_tests.conftest import login_user
from looperget.tests.software_tests.factories import UserFactory
...
def redirects_to_login_page(testapp, endpoint):
    ...
    assert "Looperget Login" in response
...
@mock.patch('looperget.looperget_flask.routes_authentication.login_log')
def test_api_logged_in_as_admin(_, testapp):
    ...
    for index, route in enumerate(api_endpoints):
        ...
        response = testapp.get(
            ...
            headers={'Accept': 'application/vnd.looperget.v1+json'})
...
def test_api_docs_when_not_logged_in(testapp):
    ...
    assert 'Looperget API' in response, "Unexpected HTTP Response: \n{body}".format(body=response.body)
...
def create_user(looperget_db, role_id, name, password):
    """Create fake admin user."""
    new_user = UserFactory()
    new_user.name = name
    new_user.set_password(password)
    new_user.role_id = role_id
    looperget_db.add(new_user)
    looperget_db.commit()
    return new_user
