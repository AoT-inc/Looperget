...
from looperget.config import (ALEMBIC_VERSION, BACKUP_LOG_FILE, CAMERA_INFO,
                           DAEMON_LOG_FILE,
                           DEPENDENCY_LOG_FILE, DOCKER_CONTAINER,
                           FRONTEND_PID_FILE, HTTP_ACCESS_LOG_FILE,
                           HTTP_ERROR_LOG_FILE, IMPORT_LOG_FILE,
                           KEEPUP_LOG_FILE, LOGIN_LOG_FILE, LOOPERGET_DB_PATH,
                           LOOPERGET_VERSION, RESTORE_LOG_FILE, THEMES_DARK,
                           UPGRADE_LOG_FILE)
from looperget.config_devices_units import MEASUREMENTS
from looperget.databases.models import (PID, AlembicVersion, Camera, Conversion,
...
from looperget.looperget_client import DaemonControl, daemon_active
from looperget.looperget_flask.extensions import db
from looperget.looperget_flask.forms import forms_camera, forms_misc, forms_notes
from looperget.looperget_flask.routes_static import inject_variables
from looperget.looperget_flask.utils import (utils_camera, utils_dashboard,
...
from looperget.utils.inputs import (list_analog_to_digital_converters,
                                 parse_input_information)
from looperget.utils.outputs import output_types, parse_output_information
from looperget.utils.system_pi import (
...
from looperget.utils.tools import (calc_energy_usage, return_energy_usage,
                                return_output_usage)

logger = logging.getLogger('looperget.looperget_flask.routes_page')
...
def page_camera():
    ...
    for each_cam in camera:
        if each_cam.library == 'opencv':
            try:
                from looperget.devices.camera import count_cameras_opencv
            ...
            if mod_camera.stream_started:
                try:
                    camera_stream = import_module(
                        'looperget.looperget_flask.camera.camera_{lib}'.format(
        ...
        if form_note_show.notes_show.data:
            ...
            if data:
                ...
                return send_file(
                    ...
                    as_attachment=True,
                    download_name=
                    'Looperget_Notes_{mv}_{host}_{dt}.zip'.format(
                        mv=LOOPERGET_VERSION,
...
def page_info():
    ...
    return render_template('pages/info.html',
                           ...
                           database_url=LOOPERGET_DB_PATH,
                           df=df_output,
...
def page_logview():
    ...
    if form_log_view.log.data == 'log_nginx':
        if DOCKER_CONTAINER:
            command = f'docker logs -n {lines} looperget_nginx'
            if search:
        ...
        if DOCKER_CONTAINER:
            command = f'docker logs -n {lines} looperget_flask'
            if search:
                command += f' 2>&1 | grep "{search}"'
        else:
            command = f'journalctl -u loopergetflask -n {lines} --no-pager'
            if search:
                command += f' | grep "{search}"'
