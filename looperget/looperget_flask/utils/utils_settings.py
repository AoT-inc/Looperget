...
from looperget.config import DAEMON_LOG_FILE
from looperget.config import DEPENDENCY_INIT_FILE
from looperget.config import DEPENDENCY_LOG_FILE
from looperget.config import INSTALL_DIRECTORY
from looperget.config import PATH_ACTIONS_CUSTOM
from looperget.config import PATH_FUNCTIONS_CUSTOM
from looperget.config import PATH_INPUTS_CUSTOM
from looperget.config import PATH_OUTPUTS_CUSTOM
from looperget.config import PATH_TEMPLATE_USER
from looperget.config import PATH_WIDGETS_CUSTOM
from looperget.config import UPGRADE_INIT_FILE
from looperget.config_devices_units import MEASUREMENTS
from looperget.config_devices_units import UNITS
from looperget.config_translations import TRANSLATIONS
from looperget.databases import set_api_key
...
from looperget.databases.models import Role
from looperget.databases.models import SMTP
from looperget.databases.models import Unit
from looperget.databases.models import User
from looperget.databases.models import Widget
from looperget.looperget_client import DaemonControl
from looperget.looperget_flask.extensions import db
from looperget.looperget_flask.utils import utils_general
from looperget.looperget_flask.utils.utils_general import choices_measurements
from looperget.looperget_flask.utils.utils_general import choices_units
...
from looperget.looperget_flask.utils.utils_general import flash_success_errors
from looperget.utils.actions import parse_action_information
from looperget.utils.database import db_retrieve_table
...
from looperget.utils.layouts import update_layout
from looperget.utils.modules import load_module_from_file
from looperget.utils.outputs import parse_output_information
from looperget.utils.send_data import send_email
from looperget.utils.system_pi import all_conversions
from looperget.utils.system_pi import assure_path_exists
from looperget.utils.system_pi import base64_encode_bytes
from looperget.utils.system_pi import cmd_output
from looperget.utils.system_pi import set_user_grp
from looperget.utils.utils import test_password
from looperget.utils.utils import test_username
        ...
        if not messages["error"]:
            try:
                ...
                if reload_frontend:
                    ...
                    cmd = f"sleep 10 && {INSTALL_DIRECTORY}/looperget/scripts/looperget_wrapper frontend_reload 2>&1"
...
def settings_function_import(form):
    """
    Receive a function module file, check it for errors, add it to Looperget controller list
    """
    action = '{action} {controller}'.format(
    ...
    try:
        ...
        tmp_directory = os.path.join(install_dir, 'looperget/functions/tmp_functions')
        ...
        if not error:
            ...
            cmd = '{path}/looperget/scripts/looperget_wrapper frontend_reload 2>&1'.format(
                path=install_dir)
            subprocess.Popen(cmd, shell=True)
...
def settings_action_import(form):
    """
    Receive an action module file, check it for errors, add it to Looperget controller list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'looperget/actions/tmp_actions')
        assure_path_exists(tmp_directory)
...
def settings_input_import(form):
    """
    Receive an input module file, check it for errors, add it to Looperget input list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        ...
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'looperget/inputs/tmp_inputs')
        assure_path_exists(tmp_directory)
...
def settings_output_import(form):
    """
    Receive an output module file, check it for errors, add it to Looperget output list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        # correct_format = 'Looperget_LOOPERGETVERSION_Settings_DBVERSION_HOST_DATETIME.zip'
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'looperget/outputs/tmp_outputs')
        assure_path_exists(tmp_directory)
...
def settings_widget_import(form):
    """
    Receive an widget module file, check it for errors, add it to Looperget widget list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        # correct_format = 'Looperget_LOOPERGETVERSION_Settings_DBVERSION_HOST_DATETIME.zip'
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'looperget/widgets/tmp_widgets')
        assure_path_exists(tmp_directory)
    ...
    if form.enable_i2c.data:
        ...
        if form.pigpiod_sample_rate.data not in ['low', 'high', 'disabled', 'uninstalled']:
            ...
            if (form.pigpiod_sample_rate.data != 'uninstalled' and
                ...
                cmd = f"{INSTALL_DIRECTORY}/looperget/scripts/looperget_wrapper install_pigpiod" \
                      f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
                install_pigpiod = subprocess.Popen(cmd, shell=True)
                install_pigpiod.wait()

            # Disable pigpiod
            cmd = f"{INSTALL_DIRECTORY}/looperget/scripts/looperget_wrapper disable_pigpiod" \
                  f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
            ...
            if form.pigpiod_sample_rate.data == 'low':
                # Install pigpiod (sample rate of 1 ms)
                cmd = f"{INSTALL_DIRECTORY}/looperget/scripts/looperget_wrapper enable_pigpiod_low" \
                      f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
                ...
                cmd = f"{INSTALL_DIRECTORY}/looperget/scripts/looperget_wrapper enable_pigpiod_high" \
                      f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
                ...
                cmd = f"{INSTALL_DIRECTORY}/looperget/scripts/looperget_wrapper enable_pigpiod_disabled" \
                      f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
                disable_pigpiod = subprocess.Popen(cmd, shell=True)
                disable_pigpiod.wait()
            elif form.pigpiod_sample_rate.data == 'uninstalled':
                # Uninstall pigpiod (user selected disable)
                cmd = f"{INSTALL_DIRECTORY}/looperget/scripts/looperget_wrapper uninstall_pigpiod" \
                      f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
                uninstall_pigpiod = subprocess.Popen(cmd, shell=True)
                uninstall_pigpiod.wait()

            # Start the Looperget daemon
            cmd = f"{INSTALL_DIRECTORY}/looperget/scripts/looperget_wrapper daemon_start" \
                  f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
        ...
        if form_mod_alert.validate():
            ...
            if form_mod_alert.send_test.data:
                send_email(
                    ...
                    "This is a test email from Looperget")
...
def settings_diagnostic_delete_settings_database():
    ...
    if not error:
        try:
            os.remove('/opt/Looperget/databases/looperget.db')
            cmd = "{pth}/looperget/scripts/looperget_wrapper frontend_reload" \
                  " | ts '[%Y-%m-%d %H:%M:%S]'" \
...
def settings_diagnostic_recreate_influxdb_db_1():
    ...
    if not error:
        try:
            command = f'/bin/bash {INSTALL_DIRECTORY}/looperget/scripts/upgrade_commands.sh recreate-influxdb-1-db'
...
def settings_diagnostic_recreate_influxdb_db_2():
    ...
    if not error:
        try:
            command = f'/bin/bash {INSTALL_DIRECTORY}/looperget/scripts/upgrade_commands.sh recreate-influxdb-2-db'
            p = subprocess.Popen(command, shell=True)
            p.communicate()
        except Exception:
            logger.exception()
    ...
    if not error:
        try:
            def install_dependencies():
                cmd = "{pth}/looperget/scripts/looperget_wrapper update_dependencies" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' >> {log} 2>&1".format(
                    pth=INSTALL_DIRECTORY,
                    log=DEPENDENCY_LOG_FILE)
                _, _, _ = cmd_output(cmd, user="root")

                cmd = "{pth}/looperget/scripts/looperget_wrapper frontend_restart" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' >> {log} 2>&1".format(
                    pth=INSTALL_DIRECTORY,
                    log=DEPENDENCY_LOG_FILE)
                _, _, _ = cmd_output(cmd, user="root")

                cmd = "{pth}/looperget/scripts/looperget_wrapper daemon_restart" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' >> {log} 2>&1".format(
                    pth=INSTALL_DIRECTORY,
                    log=DEPENDENCY_LOG_FILE)
                _, _, _ = cmd_output(cmd, user="root")
...
def settings_regenerate_widget_html():
    try:
        ...
        cmd = f'/bin/bash {INSTALL_DIRECTORY}/looperget/scripts/upgrade_commands.sh generate-widget-html'
...
def settings_diagnostic_upgrade_master():
    ...
    if not error:
        try:
            path_config = os.path.join(INSTALL_DIRECTORY, "looperget/config.py")
            ...
            set_user_grp(path_config, 'looperget', 'looperget')
            flash_success_errors(
                error, action, url_for('routes_settings.settings_diagnostic'))
            return
        except Exception as except_msg:
            ...
            command = '/bin/bash {path}/looperget/scripts/upgrade_commands.sh web-server-restart'.format(
