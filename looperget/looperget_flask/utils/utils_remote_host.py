...
from looperget.config import STORED_SSL_CERTIFICATE_PATH
from looperget.databases.models import DisplayOrder
from looperget.databases.models import Remote
from looperget.looperget_flask.extensions import db
from looperget.looperget_flask.utils import utils_general
from looperget.looperget_flask.utils.utils_general import add_display_order
...
from looperget.utils.system_pi import csv_to_list_of_str
from looperget.utils.system_pi import list_to_csv

logger = logging.getLogger(__name__)

#
# Remote host commands executed on Looperget with Remote Admin Dashboard
#

def remote_log_in(address, user, password_hash):
    """
    Log in to Looperget and return the cookie header for subsequent requests

    :param address: host name or IP address of remote Looperget
    :param user: User name of an admin on the remote Looperget
...
def remote_host_page(address, headers, page):
    """
    Make request, receive response, and authenticate a remote Looperget
...
def remote_host_add(form_setup, display_order):
    """
    Add a remote Looperget to the Remote Admin Dashboard

    Authenticate a remote Looperget install that will be used on this system's
    ...
    if form_setup.validate():
        try:
            # Send user and password to remote Looperget to authenticate
            credentials = {
...
def remote_host_del(form_setup):
    """Delete a remote Looperget from the Remote Admin Dashboard."""
    if not utils_general.user_has_permission('edit_settings'):
        return redirect(url_for('routes_general.home'))
