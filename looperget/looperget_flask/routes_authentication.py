...
from looperget.config import (INSTALL_DIRECTORY, LANGUAGES, LOGIN_ATTEMPTS,
                           LOGIN_BAN_SECONDS, LOGIN_LOG_FILE)
from looperget.config_translations import TRANSLATIONS
from looperget.databases.models import AlembicVersion, Misc, Role, User
from looperget.looperget_flask.extensions import db
from looperget.looperget_flask.forms import forms_authentication
from looperget.looperget_flask.utils import utils_general
from looperget.utils.utils import test_password, test_username
...
def create_admin():
    ...
    # Find user-selected language in Looperget/.language
    try:
    ...
    if banned_from_login():
        ...
        if request.method == 'POST':
            if form_language.language.data:
                ...
                username = form_login.looperget_username.data.lower()
                ...
                if not user:
                    ...
                    matched_hash = User().check_password(
                        form_login.looperget_password.data, user.password_hash)
...
def newremote():
    ...
    if user:
        if User().check_password(
            ...
            try:
                with open('/opt/Looperget/looperget/looperget_flask/ssl_certs/cert.pem', 'r') as cert:
...
def check_database_version_issue():
    if len(AlembicVersion.query.all()) > 1:
        flash("A check of your database indicates there is an issue with your"
              ...
              " your looperget.db from /opt/Looperget/databases/looperget.db to a "
