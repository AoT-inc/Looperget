...
Inputs, such as sensors, ADC signals, or even a response from a command, enable measuring conditions in the environment or elsewhere, which will be stored in a time-series database (InfluxDB). This database will provide measurements for [Dashboard Widgets](Data-Viewing.md#dashboard), [Functions](Functions.md), and other parts of Looperget to operate from. Add, configure, and activate inputs to begin recording measurements to the database and allow them to be used throughout Looperget.

### Custom Inputs

See [Building a Custom Input Module](https://github.com/aot-inc/Looperget/wiki/Building-a-Custom-Input-Module) Wiki page.

There is a Custom Input import system in Looperget that allows user-created Inputs to be created an used in the Looperget system. Custom Inputs can be uploaded and imported from the `[Gear Icon] -> Configure -> Custom Inputs` page. After import, they will be available to use on the `Setup -> Input` page.

If you develop a working Input module, please consider [creating a new GitHub issue](https://github.com/aot-inc/Looperget/issues/new?assignees=&labels=&template=feature-request.md&title=New%20Module) or pull request, and it may be included in the built-in set.

Open any of the built-in modules located in the directory [Looperget/looperget/inputs](https://github.com/aot-inc/Looperget/tree/master/looperget/inputs/) for examples of the proper formatting.

There are also example Custom Inputs in the directory [Looperget/looperget/inputs/examples](https://github.com/aot-inc/Looperget/tree/master/looperget/inputs/examples)
...
The full code used to decode both `bme280_ttn.py` and `k30_ttn.py`, with informative comments, is located at `/opt/Looperget/looperget/inputs/examples/ttn_data_storage_decoder_example.js`.

These example Input modules may be modified to suit your needs and imported into Looperget through the `[Gear Icon] -> Configure -> Custom Inputs` page. After import, they will be available to use on the `Setup -> Input` page.